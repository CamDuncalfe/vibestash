...
export function ProductCard({ product, isPotd }: { product: Product; showFeaturedBadge?: boolean; isPotd?: boolean }) {
  const [isHovering, setIsHovering] = useState(false);
  const isNew = !isPotd && Date.now() - new Date(product.created_at).getTime() < 48 * 60 * 60 * 1000;
      ...
      >
        ...
        >
          <div className="border-preview">
            {isPotd && (
              <span className="absolute top-2 left-2 z-10 bg-amber-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-1">
